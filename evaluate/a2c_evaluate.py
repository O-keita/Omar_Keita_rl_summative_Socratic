...
def find_model(model_path_hint: str):
    if model_path_hint:
        ...
        if p.exists():
            return str(p)
    base = Path("../models/a2c_engagement")
    candidates = []
    if base.exists():
...
def save_results_csv(results, out_path="eval_results.csv"):
    if not results:
        return
...
def main():
    ...
    parser.add_argument("--out-csv", type=str, default="results/a2c_eval_results.csv")
    args = parser.parse_args()

    model_path = find_model(args.model_path)
    if model_path is None:
        raise FileNotFoundError("Could not find a model. Check --model-path or ../models/a2c_engagement/")
